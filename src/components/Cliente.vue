        ...
        <v-container>
          ...
          <v-container>
            <v-responsive max-width="550" class="d-flex justify-center">
              <v-text-field color="#ffff" v-model="search" append-icon="mdi-magnify" label="Buscar" single-line hide-details></v-text-field>
            </v-responsive>
          </v-container>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ props }">
            <v-btn dark class="mb-2" v-bind="props"
              style="border-radius: 5px; border-width: 1px; color: #ffff ;background: linear-gradient(45deg, rgb(108 56 255), rgb(124 77 255))">+ Crear nuevo </v-btn>
          </template>
          <v-card>
            <v-card-title>
